...
_ , ymax = plt.ylim()
fig.tight_layout()
plt.savefig('results/'+name+'_grmp.pdf', bbox_inches='tight')
plt.show()
...
for i,graph in enumerate(graphs):
    ...
    ax.set_aspect('equal')
    fig.tight_layout()
    plt.savefig('results/'+name+'_init_'+str(i)+'.pdf', bbox_inches='tight')
    plt.show()
    ...
    if G_on_projection:
        ...
        ax.set_aspect('equal')
        fig.tight_layout()
        plt.savefig('results/'+name+'_init_G_on_projection_'+str(i)+'.pdf', bbox_inches='tight')
        plt.show()
...
for i,graph in enumerate(graphs):
    ...
    ax.set_yticks([])
    fig.tight_layout()
    plt.savefig('results/'+name+'_filtered_'+str(i)+'.pdf', bbox_inches='tight')
    plt.show()
    ...
    if G_on_projection:
        ...
        fig.tight_layout()
        plt.savefig('results/'+name+'_filtered_G_on_projection_'+str(i)+'.pdf', bbox_inches='tight')
        plt.show()
